...
void UserEditDlg::setDeptTreeSelect(const QList<std::shared_ptr<TreeSelect>>& deptTreeSelect)
{
	ui->deptCmb->clear();
...
void UserEditDlg::setPosts(const QList<std::shared_ptr<Post>>& posts)
{
...
void UserEditDlg::setRoles(const QList<std::shared_ptr<Role>>& roles)
{
...
void UserEditDlg::setSelectPosts(const QList<qint32>& postIds)
{
	for (auto postId : postIds) {
		auto row = ui->postTagPicker->findData(postId);
		if (row != -1) {
			ui->postTagPicker->selectTag(row);
		}
	}
}

void UserEditDlg::setSelectRoles(const QList<qint32>& roleIds)
{
	for (auto roleId : roleIds) {
		auto row = ui->roleTagPicker->findData(roleId);
		if (row != -1) {
			ui->roleTagPicker->selectTag(row);
		}
	}
}

void UserEditDlg::setUser(const std::shared_ptr<User>& user)
{
	if (!user) {
	...
	ui->phoneEdit->setText(m_user->phone_number);
	ui->sexCmb->setCurrentIndex(m_user->sex);

	auto index = ui->deptCmb->findData(m_user->dept_id, [=](const QVariant& data, const QVariant& value) {
		auto p = data.value<std::shared_ptr<TreeSelect>>();
		if (p) {
			return p->id == value.toInt();
		}
		return false;
		});
	if(index.isValid())
		ui->deptCmb->setCurrentIndex(index);
	...
	ui->remarkEdit->setPlainText(m_user->remark);
}

void UserEditDlg::on_okBtn_clicked()
{
	...
	HttpMgr::instance()->put(URL("/user/edit"), juser, ReqId::user_edit, Modules::UserMgr);
}
